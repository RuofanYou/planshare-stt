...
const loginLimiter = createFixedWindowLimiter({ limit: 5, windowMs: 15 * 60 * 1000 })
const likeIpLimiter = createFixedWindowLimiter({ limit: 60, windowMs: 60 * 1000 })
const likeBoardLimiter = createFixedWindowLimiter({ limit: 5, windowMs: 60 * 1000 })
const submissionLimiter = createFixedWindowLimiter({ limit: 10, windowMs: 60 * 60 * 1000 })
...
function seedIfEmpty() {
  ...
  const insertAuthor = db.prepare(`
    INSERT INTO authors (id, name, avatar_url, bio, guild_name, guild_recruit, guild_contact)
    VALUES (@id, @name, @avatarUrl, @bio, @guildName, @guildRecruit, @guildContact)
  `)
...
const stmt = {
  ...
  insertSubmission: db.prepare(`
    INSERT INTO submissions (
      ...
      creator_guild_contact, status, source_key, creator_user_id, created_at
    ) VALUES (
      ...
      @creatorGuildContact, 'pending', @sourceKey, @creatorUserId, @createdAt
    )
  `),
...
function createCreatorToken(userId) {
  const token = randomBytes(24).toString('hex')
  validCreatorTokens.set(token, userId)
  return token
}
...
function validateSubmissionInput(input, reply) {
  const required = [
    ['title', '标题'],
    ['raidId', '团本'],
    ['bossId', 'BOSS'],
  ...
  if (input.wantsCreatorProfile && !input.contact) {
    return reply.code(400).send({ error: '字段缺失：contact（申请创作者时必须填写联系方式）' })
  }
  return null
}
...
function getCreatorUserFromRequest(req) {
  const header = req.headers.authorization || ''
  const token = header.startsWith('Bearer ') ? header.slice(7) : ''
  const userId = token ? validCreatorTokens.get(token) : null
  return userId ? stmt.creatorUserById.get(userId) : null
}

// POST /api/admin/login -> { token }；密码错 401。
app.post('/api/admin/login', (req, reply) => {
  const clientKey = getClientKey(req)
  const limit = loginLimiter.hit(`login:${clientKey}`)
  if (!limit.allowed) {
  ...
  loginLimiter.reset(`login:${clientKey}`)
  const token = randomBytes(24).toString('hex')
...
app.get('/api/authors', () => {
  return stmt.allAuthors.all().map(rowToAuthor)
})
...
app.post('/api/boards/:id/like', (req, reply) => {
  const clientKey = getClientKey(req)
  const ipLimit = likeIpLimiter.hit(`like-ip:${clientKey}`)
  const boardLimit = likeBoardLimiter.hit(`like-board:${clientKey}:${req.params.id}`)
  if (!ipLimit.allowed || !boardLimit.allowed) {
    const resetAt = Math.max(ipLimit.resetAt, boardLimit.resetAt)
    reply.header('Retry-After', String(Math.ceil((resetAt - Date.now()) / 1000)))
...
app.post('/api/submissions', { bodyLimit: 1024 * 1024 }, (req, reply) => {
  const clientKey = getClientKey(req)
  const limit = submissionLimiter.hit(`submission:${clientKey}`)
  if (!limit.allowed) {
    reply.header('Retry-After', String(Math.ceil((limit.resetAt - Date.now()) / 1000)))
    return reply.code(429).send({ error: '投稿太频繁，请稍后再试' })
  }
...
app.post('/api/admin/import', { preHandler: requireAuth, bodyLimit: 10 * 1024 * 1024 }, (req, reply) => {
  ...
  if (!raidRows || !bossRows || !authorRows || !boardRows || !submissionRows || !creatorUserRows) return reply
