...
test('UGC smoke: browse, copy, submit, approve, publish, and creator direct post', async ({ page, request }) => {
  ...
  await expect(page.getByRole('alertdialog')).toContainText('确认清空资料草稿？')
  await page.getByRole('alertdialog').getByRole('button', { name: '取消' }).click()
  await expect(page.getByRole('alertdialog')).toHaveCount(0)
  await expect(page.getByLabel('简介')).toHaveValue(`E2E 资料草稿 ${runId}`)
  await page.getByRole('button', { name: '清空资料草稿' }).click()
  await page.getByRole('alertdialog').getByRole('button', { name: '确认清空' }).click()
  ...
  await expect(page.getByRole('alertdialog')).toContainText('确认清空直发草稿？')
  await page.getByRole('alertdialog').getByRole('button', { name: '取消' }).click()
  await expect(page.getByRole('alertdialog')).toHaveCount(0)
  await expect(page.getByLabel('战术正文')).toHaveValue('P1 创作者直发\nP2 结束')
  await page.getByRole('button', { name: '清空直发草稿' }).click()
  ...
  await expect(reloadedDirectBoardRow.getByRole('alertdialog')).toContainText('确认放弃编辑草稿？')
  await reloadedDirectBoardRow.getByRole('alertdialog').getByRole('button', { name: '取消' }).click()
  await expect(reloadedDirectBoardRow.getByRole('alertdialog')).toHaveCount(0)
  await expect(reloadedDirectBoardRow.getByLabel('标题')).toHaveValue(`${directTitle} 修订`)
  await reloadedDirectBoardRow.getByRole('button', { name: '放弃编辑草稿' }).click()
  await reloadedDirectBoardRow.getByRole('alertdialog').getByRole('button', { name: '确认放弃' }).click()
  ...
  await expect(reloadedDirectBoardRow.getByRole('alertdialog')).toHaveCount(0)
  await reloadedDirectBoardRow.getByRole('button', { name: '下架' }).click()
  await reloadedDirectBoardRow.getByRole('alertdialog').getByRole('button', { name: '确认下架' }).click()
  ...
  await expect(reloadedDirectBoardRow.getByRole('alertdialog')).toContainText('确定恢复发布这个战术板？')
  await reloadedDirectBoardRow.getByRole('alertdialog').getByRole('button', { name: '取消' }).click()
  await expect(reloadedDirectBoardRow.getByText('已下架')).toBeVisible()
  await expect(reloadedDirectBoardRow.getByRole('alertdialog')).toHaveCount(0)
  await reloadedDirectBoardRow.getByRole('button', { name: '恢复发布' }).click()
  await reloadedDirectBoardRow.getByRole('alertdialog').getByRole('button', { name: '确认恢复' }).click()
  await expect(reloadedDirectBoardRow.getByText('已发布')).toBeVisible()
...
test('admin can reset a creator password and the creator can log in again', async ({ page, request }) => {
  ...
  await accountRow.getByRole('button', { name: '隐藏新密码' }).click()
  await expect(accountRow.getByText(`已重置。请把新密码「${resetPassword}」发给用户；离开本行后后台不会再显示它。`)).toHaveCount(0)
  await expect(accountRow.getByText('新密码已复制。')).toHaveCount(0)
...
test('creator can fix and retry a rejected submission from dashboard', async ({ page, request }) => {
  ...
  await expect(rejectedRow.getByRole('alertdialog')).toContainText(`确认驳回「${creator.title}」？`)
  await rejectedRow.getByRole('button', { name: '取消' }).click()
  await expect(rejectedRow.getByText('待审核')).toBeVisible()
  await expect(rejectedRow.getByRole('alertdialog')).toHaveCount(0)
  await rejectedRow.getByRole('button', { name: '驳回' }).click()
  await rejectedRow.getByRole('button', { name: '确认驳回' }).click()
...
test('creator can fix and retry an admin-spammed submission from dashboard', async ({ page, request }) => {
  ...
  await expect(spamRow.getByRole('alertdialog')).toContainText(`确认把「${creator.title}」标记为垃圾？`)
  await spamRow.getByRole('button', { name: '取消' }).click()
  await expect(spamRow.getByText('待审核')).toBeVisible()
  await expect(spamRow.getByRole('alertdialog')).toHaveCount(0)
  await spamRow.getByRole('button', { name: '标记垃圾' }).click()
  await spamRow.getByRole('button', { name: '确认标记垃圾' }).click()
...
test('submit draft survives reload without saving password or contact', async ({ page }) => {
  ...
  await expect(page.getByRole('alertdialog')).toContainText('确认清空本机草稿？')
  await page.getByRole('alertdialog').getByRole('button', { name: '取消' }).click()
  await expect(page.getByRole('alertdialog')).toHaveCount(0)
  await expect(page.getByLabel('战术正文')).toHaveValue(`P1 草稿保护 ${runId}\nP2 集合`)
  await page.getByRole('button', { name: '清空草稿' }).click()
  ...
  await expect(page.getByRole('link', { name: '回首页浏览' })).toBeVisible()
  await page.getByLabel('标题').fill(`下一份投稿 ${runId}`)
  await expect(page.getByText(/投稿已进入审核，不会立刻公开/)).toHaveCount(0)
  await expect(page.getByRole('status').getByText('已复制投稿编号。')).toHaveCount(0)
  await expect(page.getByText('还差：团本、BOSS、投稿署名、战术正文')).toBeVisible()
  await page.getByRole('button', { name: '清空草稿' }).click()
  await page.getByRole('alertdialog').getByRole('button', { name: '确认清空' }).click()
  await page.goto('/')
...
test('visitor can report a board and admin can hide it from public pages', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: '取消举报' }).click()
  await expect(page.getByRole('button', { name: '提交举报' })).toHaveCount(0)
  await expect(page.getByText('选择其它原因时请补充说明。')).toHaveCount(0)
  await page.getByRole('button', { name: '举报' }).click()
  await expect(page.locator('#ps-report-reason')).toHaveValue('wrong-info')
  await page.locator('#ps-report-reason').selectOption('other')
  ...
  await expect(reportRow.getByText('待处理')).toBeVisible()
  await reportRow.getByRole('button', { name: '隐藏板' }).click()
  await expect(reportRow.getByRole('alertdialog')).toContainText(`确认隐藏「${board.title}」？`)
  await reportRow.getByRole('button', { name: '取消' }).click()
  await expect(reportRow.getByRole('alertdialog')).toHaveCount(0)
  await expect(reportRow.getByText('待处理')).toBeVisible()
  await reportRow.getByRole('button', { name: '隐藏板' }).click()
  await reportRow.getByRole('button', { name: '确认隐藏' }).click()
